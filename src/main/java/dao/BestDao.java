...
public class BestDao {
	...
	// Index에 인기 순위를 보여주는 메서드
		public List<Map<String,Object>> selectBestListByPageIndex(){
			...
			ResultSet rs = null;
			// order 상품 판매 주문순으로 15개 나열
			String sql = " SELECT p.product_no, p.name, p.price,p.picture ,p.volume,p.alcohol_level, o.product_no, RANK() over(order BY o.cnt DESC) ranking FROM (SELECT product_no,COUNT(*) cnt FROM `order` GROUP BY product_no) o INNER JOIN product p ON p.product_no = o.product_no LIMIT 0, 3 ";
